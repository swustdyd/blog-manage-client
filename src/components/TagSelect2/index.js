...
export default class TagSelect2 extends React.Component{
    ...
    renderTags(tags){
        ...
        const renderTags = tags.map((tag) => {
            ...
            return <span key={tag.id} className={className} style={style} onClick={() => this.handleTagClick(tag)}>{tag.name}</span>
        })
        const allClassName = `${styles.tag}${selectAll ? ` ${styles.tagSelected}` : ''}`;
        return [<span key="all" onClick={() => this.handleSelectAllClick()} className={allClassName}>{allText || '全部'}</span>].concat(renderTags);
    }
            ...
            <div className={styles.container}>
                ...
                {expandable ? (
                    <div className={styles.expandContainer}>
                        <span onClick={() => this.handleExpandClick()}>{expand ? <Icon type="up" /> : <Icon type="down" />}&nbsp;{expandText}</span>
                    </div>
                ) : null}
            </div>
        )
